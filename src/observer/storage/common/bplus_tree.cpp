...
RC BplusTreeHandler::print() {
    ...
    for (i = 1; i <= page_count; i++) {
        ...
        for (j = 0; j < node->key_num && j < 6; j++) {
            printf("keynum :%d rids:page_num :%d,slotnum :%d\n", node->key_num, node->rids[j].page_num,
                   node->rids[j].slot_num);
        }
        printf("\n");
        rc = disk_buffer_pool_->unpin_page(&page_handle);
        if (rc != SUCCESS) {
...
RC BplusTreeHandler::insert_into_leaf_after_split(PageNum leaf_page, const char *pkey, const RID *rid) {
    ...
    for (i = 0, j = 0; i < leaf->key_num; i++, j++) {
        if (j == insert_pos)
            j++;
        memcpy(temp_keys + j * file_header_.key_length, leaf->keys + i * file_header_.key_length,
    ...
    for (i = 0; i < split; i++) {
        memcpy(leaf->keys + i * file_header_.key_length, temp_keys + i * file_header_.key_length,
               file_header_.key_length);
    ...
    for (i = split, j = 0; i < file_header_.order; i++, j++) {
        memcpy(new_node->keys + j * file_header_.key_length, temp_keys + i * file_header_.key_length,
               file_header_.key_length);
...
RC BplusTreeHandler::insert_intern_node(PageNum parent_page, PageNum left_page, PageNum right_page, const char *pkey) {
    ...
    for (i = node->key_num; i > insert_pos; i--) {
        ...
        memcpy(node->keys + i * file_header_.key_length, node->keys + (i - 1) * file_header_.key_length,
               file_header_.key_length);
    }
...
RC BplusTreeHandler::insert_intern_node_after_split(PageNum inter_page, PageNum left_page, PageNum right_page,
                                                    const char *pkey) {
    RC rc;
    ...
    for (i = 0, j = 0; i < inter_node->key_num; i++, j++) {
        if (j == insert_pos)
            j++;
        memcpy(temp_keys + j * file_header_.key_length, inter_node->keys + i * file_header_.key_length,
               file_header_.key_length);
    }
    ...
    for (i = 0; i < split - 1; i++) {
        memcpy(inter_node->keys + i * file_header_.key_length, temp_keys + i * file_header_.key_length,
               file_header_.key_length);
...
RC BplusTreeHandler::get_entry(const char *pkey, RID *rid) {
    ...
    for (i = 0; i < leaf->key_num; i++) {
        if (CmpKey(file_header_.attr_type, file_header_.attr_length, key, leaf->keys + (i * file_header_.key_length)) ==
            0) {
...
RC BplusTreeHandler::coalesce_node(PageNum leaf_page, PageNum right_page) {
    ...
    if (left->is_leaf == false) {
        memcpy(left->keys + start * file_header_.key_length, parent->keys + k * file_header_.key_length,
               file_header_.key_length);
        start++;
        left->key_num++;
    }
    for (i = start, j = 0; j < right->key_num; i++, j++) {
        memcpy(left->keys + i * file_header_.key_length, right->keys + j * file_header_.key_length,
               file_header_.key_length);
    ...
    if (left->is_leaf) {
        ...
        if (left->key_num < min_key) {
            ...
            for (i = 0; i < right->key_num - 1; i++) {
                memcpy(right->keys + i * file_header_.key_length, right->keys + (i + 1) * file_header_.key_length,
                       file_header_.key_length);
        ...
        if (left->key_num < min_key) {
            memcpy(left->keys + left->key_num * file_header_.key_length, parent->keys + k * file_header_.key_length,
                   file_header_.key_length);
            ...
            right->key_num++;
            memcpy(parent->keys + k * file_header_.key_length,
                   left->keys + (left->key_num - 1) * file_header_.key_length, file_header_.key_length);
            left->key_num--;
...
RC BplusTreeHandler::print_tree() {
    ...
    while (page_num != 0) {
        for (i = 0; i < node->key_num; i++) {
            ...
            printf("key : %d,rids (page_num:%d slotnum %d)\n", *(int *) pkey, node->rids[i].page_num,
                   node->rids[i].slot_num);
        }
        printf("next node:%d\n", page_num);
        rc = disk_buffer_pool_->unpin_page(&page_handle);
        if (rc != SUCCESS) {
...
RC BplusTreeScanner::find_idx_pages() {
    ...
    for (int i = 0; i < num_fixed_pages_; i++) {
        if (next_page_num_ <= 0)
            break;
        rc = index_handler_.disk_buffer_pool_->get_this_page(index_handler_.file_id_, next_page_num_,
                                                             page_handles_ + i);
        if (rc != SUCCESS) {
            return rc;
        }
